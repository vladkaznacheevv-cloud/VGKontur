...
- `lib/` — `contacts` (бренд/контакты), `estimate` (временная таблица ставок + расчёт), `leadValidation` (нормализация/санитайз), `telegram` (формат + отправка), `metrica` + `metricaGoals` (цели), `serviceArea` (города КМВ).
...
- Точные финальные цены не показывать. Калькулятор даёт только «предварительную оценку по площади и типу работ»; временные ставки помечены `TODO: уточнить по Ставропольскому краю.`
...
- **Яндекс.Метрика не установлена**: `lib/metrica.ts` вызывает `window.ym`, но счётчик нигде не подключён (`app/layout.tsx` без `<Script>`/snippet). До установки счётчика все цели молча не срабатывают.
- Тестов в репозитории нет (папка `test-results/` — артефакт прошлого прогона, конфигов/тестов не закоммичено).
- Требует согласования до боевого запуска: финальный текст `/privacy`, реальные реквизиты оператора ПДн, боевые Telegram-креды (только в окружении деплоя).
